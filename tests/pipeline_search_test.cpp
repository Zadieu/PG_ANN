...
int main() {
  ...
  hybrid::SearchConfig cached_config = config;
  cached_config.graph_cache_budget_bytes = 8192;
  cached_config.graph_cache_policy = hybrid::GraphCacheBuildPolicy::kEntryBfs;
  hybrid::SearchStats cached_stats;
  std::cout << "stage=graph_cache_search\n" << std::flush;
  const std::vector<hybrid::SearchResult> cached_results = searcher.Search(query, cached_config, &cached_stats);
  hybrid::SearchConfig page_layout_config = cached_config;
  page_layout_config.graph_cache_policy = hybrid::GraphCacheBuildPolicy::kPageLayout;
  hybrid::SearchStats page_layout_stats;
  std::cout << "stage=page_layout_graph_cache_search\n" << std::flush;
  const std::vector<hybrid::SearchResult> page_layout_results =
      searcher.Search(query, page_layout_config, &page_layout_stats);
  hybrid::SearchConfig two_stage_config = cached_config;
  two_stage_config.defer_exact_until_refinement = true;
  two_stage_config.refine_k = config.top_k;
  hybrid::SearchStats two_stage_stats;
  std::cout << "stage=two_stage_refinement_search\n" << std::flush;
  const std::vector<hybrid::SearchResult> two_stage_results =
      searcher.Search(query, two_stage_config, &two_stage_stats);
  hybrid::SearchConfig fixed_scheduler_config = config;
  fixed_scheduler_config.scheduler_policy = hybrid::SearchConfig::SchedulerPolicy::kBounded;
  fixed_scheduler_config.scheduler_policy_limit = 2;
  fixed_scheduler_config.dynamic_beam_policy = hybrid::SearchConfig::DynamicBeamPolicy::kFixed;
  hybrid::SearchStats fixed_scheduler_stats;
  std::cout << "stage=fixed_scheduler_search\n" << std::flush;
  const std::vector<hybrid::SearchResult> fixed_scheduler_results =
      searcher.Search(query, fixed_scheduler_config, &fixed_scheduler_stats);
  ...
  assert(stats.bytes_read >= stats.pages_completed * index.search_metadata().page_size);
  assert(stats.page_resident_hits == stats.resident_expansions);
  assert(stats.exact_from_page + stats.exact_from_payload == stats.exact_distance_evals);
  assert(stats.read_hits_in_pool + stats.read_waste_out_of_pool <= stats.pages_completed);
  assert(stats.refinement_candidates > 0);
  assert(stats.max_inflight_reads > 0);
  assert(stats.max_beam_width <= config.beam_width);
  ...
  assert(cached_results.front().id == results.front().id);
  assert(cached_stats.graph_cache_entries > 0);
  assert(cached_stats.graph_cache_resident_bytes > 0);
  assert(cached_stats.graph_cache_hits > 0);
  assert(cached_stats.graph_cache_avoided_reads > 0);
  assert(cached_stats.async_reads >= cached_stats.refinement_reads);
  assert(stats.async_reads >= stats.refinement_reads);
  assert(cached_stats.async_reads - cached_stats.refinement_reads <= stats.async_reads - stats.refinement_reads);
  assert(!page_layout_results.empty());
  assert(page_layout_results.front().id == results.front().id);
  assert(page_layout_stats.graph_cache_entries > 0);
  assert(page_layout_stats.graph_cache_hits > 0);
  assert(!two_stage_results.empty());
  assert(two_stage_results.front().id == results.front().id);
  assert(two_stage_stats.deferred_exact_candidates > 0);
  assert(two_stage_stats.refinement_bound >= two_stage_config.top_k);
  assert(two_stage_stats.approximate_candidates >= two_stage_stats.refinement_bound);
  assert(two_stage_stats.refinement_exactified > 0);
  assert(two_stage_stats.exact_distance_evals >= two_stage_results.size());
  assert(!fixed_scheduler_results.empty());
  assert(fixed_scheduler_results.front().id == results.front().id);
  assert(fixed_scheduler_stats.scheduler_policy_limit >= fixed_scheduler_config.scheduler_policy_limit);
  assert(fixed_scheduler_stats.scheduler_pending_max > 0);
  assert(fixed_scheduler_stats.beam_width_increases == 0);
  assert(fixed_scheduler_stats.max_beam_width == fixed_scheduler_config.beam_width);
