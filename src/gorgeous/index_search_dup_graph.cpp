...
namespace diskann {
  ...
  struct CacheNode {
    ...
    unsigned nb_size;
    unsigned* node_nbrs;
    std::vector<unsigned> owned_nbrs;
    ...
    CacheNode(unsigned id, const std::vector<unsigned>& node_nbrs) {
      this->id = id;
      this->owned_nbrs = node_nbrs;
      this->nb_size = (unsigned) this->owned_nbrs.size();
      this->node_nbrs = this->owned_nbrs.data();
    }
  };
              ...
              if (retset[marker].flag) {
                ...
                if (mem_pos != INF) {
                  if (dynamic_graph_cache_size == 0 || mem_pos < dynamic_graph_cache_start) {
                    ...
                    std::vector<unsigned> cached_nbrs;
                    if (copy_mem_graph_neighbors(id, mem_pos, cached_nbrs)) {
                      cached_node.push(std::make_shared<CacheNode>(id, cached_nbrs));
                      if (disk_batch_size > 0)
                        disk_seen++;
                      else
                        num_seen++;
                ...
                if (mem_pos == INF && loaded.find(id) != loaded.end()) {
