  ...
  void DecoIndex<T>::page_search(
    ...
    pool->runTask([&, this](int tid) {
      ...
      while(true) {
        ...
        auto pipeann_slow_start_width = [&]() -> _u32 {
        ...
        _u32 pipelined_graph_current_pipe_width = use_pipelined_graph_dynamic_width
            ? std::min<_u32>(pipeann_slow_start_width(),
        ...
        if (use_pipeann_dynamic_pipe_width && use_effective_pipeann_state_scheduler) {
          _u64 initial_pipe_width = pipeann_slow_start_width();
        ...
        while (num_ios < io_limit || n_io_in_q > 0 || n_proc_in_q > 0 ||
          ...
          if (use_effective_pipeann_state_scheduler || use_pipelined_graph_dynamic_width) {
          ...
          if (should_dispatch) {
            ...
            if (ftr_id < frontier.size()) {
              ...
              while(ftr_id < frontier.size()) {
                ...
                if (stats != nullptr) {
                  stats->n_ios++;
                  if (use_effective_pipeann_state_scheduler || use_pipelined_graph_io) {
