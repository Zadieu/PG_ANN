...
namespace diskann {
  ...
  class DecoIndex {
   public:
    ...
    DISKANN_DLLEXPORT unsigned node_in_mem_pos(unsigned id) {
        ...
        if (cached_popular != nullptr && cached_popular->find(id) != cached_popular->end()) {
            return cached_popular->at(id);
        }
        if (dynamic_graph_cache_size > 0) {
            std::lock_guard<std::mutex> lock(dynamic_graph_cache_mutex_);
            auto iter = dynamic_cached_nodes_.find(id);
            if (iter != dynamic_cached_nodes_.end()) {
                unsigned slot = iter->second;
                unsigned dynamic_slot = slot - dynamic_graph_cache_start;
    ...
    DISKANN_DLLEXPORT bool copy_mem_graph_neighbors(unsigned id, unsigned cache_pos,
                                                    std::vector<unsigned>& neighbors) {
        if (!use_mem_graph || cache_pos == INF || cache_pos >= mem_graph_.size()) return false;
        if (cache_pos < dynamic_graph_cache_start || dynamic_graph_cache_size == 0) {
            neighbors = mem_graph_[cache_pos];
            return true;
        }
        std::lock_guard<std::mutex> lock(dynamic_graph_cache_mutex_);
        unsigned dynamic_slot = cache_pos - dynamic_graph_cache_start;
        if (dynamic_slot >= dynamic_cache_node_ids_.size() || dynamic_cache_node_ids_[dynamic_slot] != id) {
            return false;
        }
        neighbors = mem_graph_[cache_pos];
        dynamic_cache_access_[dynamic_slot] = ++dynamic_cache_clock_;
   ...
   protected:
    ...
    DISKANN_DLLEXPORT void maybe_cache_graph_node(unsigned id, const unsigned* node_nbrs,
                                                  unsigned nnbrs);
   ...
   private:
    ...
    unsigned static_graph_cache_size = 0;
    unsigned dynamic_graph_cache_start = 0;
    unsigned dynamic_graph_cache_size = 0;
    unsigned dynamic_graph_cache_used = 0;
    unsigned dynamic_cache_clock_ = 0;
    std::vector<unsigned> dynamic_cache_node_ids_;
    std::vector<unsigned> dynamic_cache_access_;
    std::vector<unsigned> dynamic_cache_hits_;
    tsl::robin_map<unsigned, unsigned> dynamic_cached_nodes_;
    tsl::robin_map<unsigned, unsigned> dynamic_candidate_hits_;
    std::mutex dynamic_graph_cache_mutex_;
